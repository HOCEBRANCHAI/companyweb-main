      ...
      <div className="space-y-3 max-h-[320px] overflow-y-auto">
        {services.map(service => <div key={service.id} className={`border rounded-xl p-3 ${selectedServices.includes(service.name) ? 'border-pink-500 bg-pink-50' : 'border-gray-200 bg-white'}`}>
            ...
            {service.name === 'Other' && selectedServices.includes('Other') && <div className="mt-2 ml-8">
                <input type="text" value={otherService} onChange={e => setOtherService(e.target.value)} placeholder="Please specify other service" className="w-full p-2 border border-gray-300 rounded-lg text-xs bg-white text-gray-900 focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
              </div>}
          </div>)}
      </div>
